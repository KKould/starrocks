...
public class MergeCrossJoinsRuleTest {

    @Test
    public void transform() {
        MergeCrossJoinsRule rule = new MergeCrossJoinsRule();
        ...
        OptExpression crossJoin2 =
                OptExpression.create(new LogicalJoinOperator(JoinOperator.CROSS_JOIN, null), crossJoin1, t2Scan);
        OptExpression innerJoin =
                OptExpression.create(new LogicalJoinOperator(JoinOperator.INNER_JOIN, onKeys), crossJoin2, t3Scan);
        ...
        assertEquals(0, ((LogicalOlapScanOperator) joinTree.getInputs().get(1).getOp()).getTable().getId());
        assertEquals(3,
                ((LogicalOlapScanOperator) joinTree.getInputs().get(0).getInputs().get(0).getInputs().get(0).getOp()).getTable()
                        .getId());
    }
}
